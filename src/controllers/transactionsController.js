...
export async function getTransactionByUserId(req, res) {
  try {
    ...
    SELECT * FROM transaction WHERE user_id = ${userId} ORDER BY created_at
    `;
...
export async function createTransaction(req, res) {
  try {
    ...
    const transaction = await sql`
      INSERT INTO  transaction(user_id,title,amount,category) VALUES(${user_id},${title},${amount},${category})
      RETURNING *
    `;
    console.log(transaction);
    res.status(201).json(transaction[0]);
  } catch (error) {
    console.log("Error creating the tranction", error);
    res.status(500).json({ message: "Internal server error" });
  }
}
...
export async function deleteTransaction(req, res) {
  try {
    ...
    res.status(200).json({ message: "Transaction deteled successfully" });
  } catch (error) {
    console.log("Error creating the transaction", error);
    res.status(500).json({ message: "Internal server error" });
  }
